...
RayTriangleIntersection getClosestIntersection(glm::vec3 camPos, glm::vec3 rayDir, std::vector<ModelTriangle>& triangles ) {
	...
	for (int i = 0; i < triangles.size(); i++) {
		...
		if (validateIntersection(possibleSolution[1], possibleSolution[2], possibleSolution[0]*-1)) {
...
void projectImageRayTrace(DrawingWindow& window, std::vector<ModelTriangle> &vertices) {
	...
	for (size_t y = 0; y < window.height; y++) {
		for (size_t x = 0; x < window.width; x++) {
			...
			RayTriangleIntersection ray(getClosestIntersection(glm::vec3(camX, camY, camZ), rayDIR, vertices));

			window.setPixelColour(x, y, colourConvert(ray.intersectedTriangle.colour));
		}
	}
}
...
void draw(DrawingWindow &window) {
	...
	else projectImageRaster(window, faces);
